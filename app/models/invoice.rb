class Invoice < ApplicationRecord
  ...
  def self.total_sales_for(start_date, end_date)
    select('SUM(invoice_items.quantity * invoice_items.unit_price) AS revenue').joins(:transactions, :invoice_items).where("transactions.result = 'success' AND invoices.status = 'shipped' AND invoice_items.created_at BETWEEN '#{start_date} 00:00:00' AND '#{end_date} 00:00:00'")
